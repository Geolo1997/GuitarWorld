...
import pers.geolo.guitarworld.microview.*;
 ...
 * @date 2019-06-04
 */
public abstract class BaseDelegate extends BaseController {
...
//    }
    ...
    public void start(BaseDelegate toFragment) {
        MicroviewTransferUtils.start(toFragment);
    }

    public void loadRootFragment(int containerId, BaseDelegate toFragment) {
    ...
    public void startWithPop(BaseDelegate delegate) {
        MicroviewTransferUtils.start(delegate);
        ViewManager.destroy(this);
    }

    public void loadMultipleRootFragment(int id, int count, BaseDelegate[] controllers) {
    ...
    public void showHideFragment(int id, BaseDelegate delegate, BaseDelegate lastDelegate) {
        ViewGroup container = (ViewGroup) getView().findViewById(id);
        ControllerStack controllerStack = ViewManager.getControllerStack(container);
        int newI = 0, oldI = 0;
        for (int i = 0; i < controllerStack.size(); i++) {
            if (controllerStack.get(i) == delegate) {
                delegate.getView().setVisibility(View.VISIBLE);
                newI = i;
            }
            if (controllerStack.get(i) == lastDelegate) {
                lastDelegate.getView().setVisibility(View.GONE);
                oldI = i;
            }
        }
        controllerStack.set(newI, lastDelegate);
        controllerStack.set(oldI, delegate);
